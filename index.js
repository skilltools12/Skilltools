// Vercel Serverless Function adapter for SKILLTOOLS
// Set FOUNDER_CODE as a Vercel Environment Variable.
// The founder email is intentionally fixed in the server-side verification.
...
module.exports = async function handler(req, res) {
  ...
  const env = {
    FOUNDER_CODE: process.env.FOUNDER_CODE || ""
  };

  const response = await cloudflareStyleFetch(request, env);
...
function page() {
  const cards = CONTENT.map(
    ([name, icon, description, id]) => `
      <article class="card" data-category="${id}">
        <div class="icon">${escapeHtml(icon)}</div>
        <h3>${escapeHtml(name)}</h3>
        <p>${escapeHtml(description)}</p>
        <button onclick="openPage('${id}')">EN SAVOIR PLUS →</button>
      </article>
    `
  ).join("");
...
<header class="header">
  ...
  <div class="brand" id="founderTrigger">
    <b>♢ SKILLTOOLS</b>
...
<section id="home" class="page show" style="padding:0">
